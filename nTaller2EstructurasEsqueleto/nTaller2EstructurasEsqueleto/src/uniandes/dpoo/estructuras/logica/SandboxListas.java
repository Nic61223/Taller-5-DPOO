...
public class SandboxListas
{
    ...
    public void volverPositivos( )
    {
    	for(int x =0; x < listaEnteros.size();x++) {
    		if (listaEnteros.get(x)<0) {

    			int s  = listaEnteros.get(x)*-1;
    			listaEnteros.set(x, s);
    ...
    {
    	Collections.sort(listaEnteros, Comparator.reverseOrder());
    }
    ...
    public int contarEnterosRepetidos( )
    {
        ...
        for (int i = 0; i < listaEnteros.size(); i++) {
            ...
            if (visitados.contains(actual) && !repetidos.contains(actual) ) {
